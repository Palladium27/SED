...
features=pd.read_csv('C:\\Users\\raval\\soundevent\\features\\W5L4bior1.5.csv',delimiter=',',header=None)
...
model8 = Sequential([
    Dense(256, input_shape=(14,),kernel_initializer='glorot_normal'),
    Activation('relu'),
    Dense(64,kernel_initializer='glorot_normal'),
    Activation('relu'),
    Dense(32,kernel_initializer='glorot_normal'),
    Activation('tanh'),
    Dense(3,kernel_initializer='glorot_normal'),
    Activation('softmax'),
    
])

ada = Adamax(lr=0.02,decay=1e-6)
model8.compile(loss='categorical_crossentropy',
              optimizer=ada,
              metrics=['accuracy'])
#model8.fit(X_train,Y_train,batch_size=len(X),validation_data=(X_test,Y_test),epochs=100)

mdl=model8.fit(X_train,Y_train,batch_size=len(X_train),validation_data=(X_test,Y_test),epochs=500)
